...
export interface ParseOptions {
  /**
   * Testing placeholder: replace every phone with this dummy so real numbers
   * aren't exposed while testing (ANONYMIZE_PHONES=true). Re-import without it
   * to restore the actual numbers from the workbook.
   */
  dummyPhone?: string | null;
}

export function parseWorkbook(buffer: Buffer | Uint8Array, opts: ParseOptions = {}): ParseResult {
  ...
  for (const name of wb.SheetNames) {
    ...
    for (let r = hdr + 1; r < grid.length; r++) {
      ...
      records.push({
        ...
        isCompany,
        phone: dummy ? (ph.phone ? dummy : null) : ph.phone,
        size1,
        brand: clean(brand),
        quantity: qty.raw,
        size2,
